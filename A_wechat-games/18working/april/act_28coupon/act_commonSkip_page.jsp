...
<script>
	...
	var locaAddr = "${method }";
	var param = "&pharmacy_id=${pharmacyId}&user_id=${userId}&inviteType=${inviteType}&codeActId=${codeActId}";
	var wxparam = "%26codeActId%3d${codeActId}%26pharmacy_id%3d${pharmacyId}%26user_id%3d${userId}%26inviteType%3d${inviteType}%26version_sys%3dwehcat";
	var redirect_uri = 'http%3a%2f%2f' + location.host + '%2fmedhtml%2fcommon%2fcouponAct%2f'+codeActId+'%3fact_url%3d' + locaAddr + wxparam +'%26resultValue%3d' + resultValue ;
//	var redirect_uri = 'http%3a%2f%2f' + 'deve.ykd365.com' + '%2fmedhtml%2fcommon%2factivity%2f'+codeActId+'%3fact_url=' + locaAddr;
	//alert(redirect_uri);
	if (is_weixin()) {
	//	alert('https://open.weixin.qq.com/connect/oauth2/authorize?appid=' + appid + '&redirect_uri=' + redirect_uri + '&response_type=code&scope=snsapi_base&state=123#wechat_redirect');
		...
		locaAddr = 'couponAct/'+codeActId+'?act_url=' +locaAddr + '&token=' + token + '&version=' + version + param + '&resultValue=' + resultValue;
